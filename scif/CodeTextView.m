...
-(void)scrolled:(NSNotification*)notification{
    //NSLog(@"Scrolled:\n%@",[notification description]);
        if (modo == 0) {        
//            NSNumber *n_indice_inicial = [[NSNumber alloc] initWithUnsignedLong:indice_inicial];
//            
...
{    
    ...
    long editado = 0;
    do {
        ra = [[self string] lineRangeForRange:NSMakeRange(letra, 0)];
        letra = NSMaxRange(ra);
        numberOfLines++;
...
//    //        hoja_anterior = K;
//    [n setLastvisibleRange:[self getViewableRange:self]];
//    //    }
...
{
	//[gotoPanel showGoToSheet: [[self window] windowForSheet]];
}
...
-(void) turnOffWrapping
{
	...
	NSRect				frame;
	NSScrollView*		ThisscrollView = [self enclosingScrollView];
...
-(void) turnOnWrapping 
{
    ...
    [[self textContainer] setContainerSize:NSMakeSize(1000.000000, 10000000.000000)];
    ...
    [self setMaxSize:NSMakeSize(1000, 10000000)];
    [self setHorizontallyResizable:YES];
    [self setVerticallyResizable:YES];
...
-(IBAction) indentSelection: (id)sender
{
	...
	nuSelRange.length++;
	[self setSelectedRange: nuSelRange];
}
...
{
	if( sourceCode != nil && self )
	{
		[self setString: sourceCode]; // Causes recoloring notification.
		[sourceCode release];
		sourceCode = nil;
	}
	else
...
-(void)		recolorRange: (NSRange)range
{
	...
	if( self == nil || range.length == 0	// Don't like doing useless stuff.
	...
	int diff = (int)([[self textStorage] length] -(range.location +range.length));
	if( diff < 0 )
		range.length += diff;
