...
-(IBAction) createNewIdentifier: (id)sender
{
	...
	unsigned int		theRow = (unsigned int)[[self identifiers] count] -1;

	[identifiersList noteNumberOfRowsChanged];
	
    NSIndexSet * rowindex = [[NSIndexSet alloc] initWithIndex:theRow];
    
	//[identifiersList selectRow: theRow byExtendingSelection: NO];
    [identifiersList selectRowIndexes:rowindex byExtendingSelection:NO];
...
{
	return (int)[[self identifiers] count];
}
